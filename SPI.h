...
/********************************************************************************************/
/*!
 	 \brief	 it enable the clock module of the SPI by modifying the MDIS bits

 	 \param[in]  SPI_ChannelType Channel we will activated.
 	 \return when the SPI is activated
 */
static void SPI_enable(SPI_ChannelType);
/*!
 	 \brief	 It activate the clock gating

 	 \param[in]  SPI_ChannelType Channel we will activated the clock.
 	 \return when the SPI clock is activated
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel the spi that we will say if is master or slave.
 	 \param[in]  SPI_MasterType masterOrSlave Select if the SPI is master or slave.
 	 \return when the SPI channel is specified as a slave or master
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel the SPI that we will say if is master or slave.
 	 \param[in]  SPI_EnableFIFOType enableOrDisable activated the TX and RX.
 	 \return when the SPI channel is activated as TX and RX
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel we choose what channel we are modified.
 	 \param[in]  SPI_PolarityType cpol We choose the polarity of the clock.
 	 \return when the Polarity is choose
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel we choose what channel we are modified.
 	 \param[in]  frameSize Is a macro for the frameSize value.
 	 \return when the frameSize is load in the respective SPI
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel we choose what channel we are modified.
 	 \param[in]  SPI_PhaseType cpha Is the phase from the SPI.
 	 \return when the clock phase is load in the respective SPI
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel we choose what channel we are modified.
 	 \param[in]  baudRate is The baudrate we are load.
 	 \return when the baudRate is load in the respective SPI
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel we choose what channel we are modified.
 	 \param[in]  SPI_LSMorMSBType you select if is MSB or LSM.
 	 \return when we is specified LSM or MSB
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel we choose what channel we are modified.
 	 \return when the transference start is done.
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel we choose what channel we are modified.
 	 \return when the transference stop is done.
 */
...
/*!
 	 ...
 	 \param[in]  SPI_ChannelType Channel we choose what channel we are modified.
 	 \return when we send the data and clear the flag.
 */
void SPI_sendOneByte(uint8 Data);
/*!
 	 \brief	It configures the SPI for transmission, this function as arguments receives a pointer to a constant structure where are all
the configuration parameters
 	 \param[in]  SPI_ConfigType Is the SPI configuration type.
 	 \return once we initialize all SPI functions.
 */
